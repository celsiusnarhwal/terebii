...
| `TZ`                          | String   | An [IANA time zone identifier](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones). The `air_date` [template variable](#customizing-notifications) will be provided in this time zone.[^4]                                                                                                                    | No            | `UTC`                         |
...
[^4]: If you're running Terebii on a Unix-like system, you may be able to mount `/etc/localtime:/etc/localtime` in
Terebii's Docker configuration as an alternative to setting `TEREBII_TIMEZONE`.